...
// accepts text, so the app writes "● 2 busy  ● 1 needs you" and is done.
// Neither tray here takes any: the Windows notification area and the
// StatusNotifierItem protocol every Linux panel implements both accept one icon
// and a tooltip. So the information has to be drawn — a disc coloured by the
// most urgent situation present, with the count inside it.
//
...
// Rendered at every scale factor the shell asks for, rather than one bitmap
// stretched. 100%, 125%, 150% and 200% are the display scalings the Windows
// tray actually requests, and a wrong-sized tray icon is resampled by the shell
// with no filtering at all. Linux panels pick the nearest representation from
// the same set.
