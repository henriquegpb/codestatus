...
//
// The override is a test seam, and it earns its place now that both seams are
// exercised from either machine: without it, running the Windows installer
// suite on a Mac or a Linux box writes a shim and a pile of settings backups
// into a ~/AppData directory that has no business existing there.
const base = process.env.CODESTATUS_DATA_HOME
  ? path.join(process.env.CODESTATUS_DATA_HOME, 'CodeStatus')
...
const paths = {
  home,
  base,
  // NTFS preserves case and ignores it when comparing, so two spellings of one
  // path are one file. The installer needs to know: an entry of ours rewritten
  // with a different drive-letter case is still ours, and failing to recognise
  // it would leave the old entry behind firing the hook twice per event.
