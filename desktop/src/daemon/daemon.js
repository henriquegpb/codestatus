...
//
// The transport is the biggest difference from the original on Windows, where
// it is a named pipe rather than a Unix domain socket; on Linux it is the same
// AF_UNIX socket the macOS app uses. Either way the wire format is the same
// NDJSON and the semantics are the same too — one connection per event, the
// hook writes and exits — so this file only ever asks platform/transport for an
// endpoint and never learns which kind it got.
//
// The other difference is how a dead process is noticed. macOS gets kqueue,
// which tells the app the moment a pid exits. Neither platform here has an
// equivalent that is cheap from Node, so liveness is polled. Nothing about
// state depends on the polling interval: a session is only ever removed because
// its process is gone, never because it went quiet.
...
const transport = require('../platform/transport');
...
// Slow on purpose, and paced for the more expensive of the two implementations:
// on Windows the scan shells out to PowerShell and costs a few hundred
// milliseconds, while on Linux it is a walk over /proc and costs almost
// nothing. Nothing time-critical depends on it either way — hooks arrive over
// the socket regardless — so it only has to be fast enough to explain a silent
// agent before the user gives up on the app.
...
class Daemon extends EventEmitter {
  ...
  listen() {
    ...
    // A filesystem socket outlives the process that bound it, so a daemon that
    // was killed rather than closed leaves one behind and the next bind fails
    // with EADDRINUSE — reported as though a second copy were running. Clearing
    // it is safe here because the caller has already established it is the only
    // instance. On Windows this is a no-op: a pipe name dies with its handles.
    transport.prepare(this.endpoint);

    server.listen(this.endpoint, () => {
      // Same-user only, where the platform needs to be told; see the seam.
      transport.finalize(this.endpoint);
      // The on-disk pointer exists so the hook can find the endpoint without
      // having it compiled in — and, on Linux, without reconstructing a runtime
      // directory it may not have the environment to reconstruct. The name can
      // change without rewriting any agent config.
      try {
        fs.writeFileSync(paths.pipePointer, this.endpoint, 'utf8');
      } catch { /* without the pointer the hook falls back to the spool */ }
      this.emit('listening', this.endpoint);
    });
