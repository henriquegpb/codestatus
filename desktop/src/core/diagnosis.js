...
// and the platform records when each process started — Win32_Process on one,
// /proc/<pid>/stat against /proc/stat's btime on the other.
//
// The macOS build has a third cause — Codex refusing to run hooks it has not
// been trusted with. Codex is not installed by this build on either platform,
// so that case has no equivalent here yet and is deliberately absent rather
// than stubbed. Its
