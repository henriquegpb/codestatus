...
// Which terminal a session is sitting in. Display only: it labels the row, and
// nothing about state or focus depends on it.
//
// Neither of these platforms has the single convention macOS does, where
// TERM_PROGRAM is set by every terminal worth naming. Windows terminals export
// almost nothing; Linux ones each export something different. Both seams
// therefore answer this twice — from the environment in the hook, and from the
// process tree in the daemon — and see platform/<os>/host.js for each.
//
// The list is not one value per terminal. A name earns a value here by being
// one a user would recognise on a row; everything else we can positively
// identify as a terminal lands on `terminal`, which is more honest than
// `unknown` and does not turn this enum into a catalogue.
...
function hostDisplayName(host) {
  switch (host) {
    ...
    case HostApplication.konsole: return 'Konsole';
    case HostApplication.gnomeTerminal: return 'Terminal';
    case HostApplication.xfceTerminal: return 'Terminal';
    case HostApplication.kitty: return 'kitty';
    case HostApplication.alacritty: return 'Alacritty';
    case HostApplication.wezterm: return 'WezTerm';
    case HostApplication.terminal: return 'Terminal';
    default: return '';
  }
}
