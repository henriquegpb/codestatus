...
const SETTINGS_STATE = {
  ...
  openAtLogin: false,
  // Previewed as the platform this is being run on, so the wording the seam
  // changes — the autostart label, what the scan costs, the focus limitation —
  // is checked rather than assumed.
  platform: process.platform,
  focusLimitation: process.platform === 'linux'
    ? 'This is a Wayland session. Terminals running through XWayland can be raised; '
      + 'native Wayland windows cannot, and those fall back to opening the folder.'
    : null,
  hooks: process.platform === 'win32'
    ? {
      installed: true,
      settingsPath: path.join(os.homedir(), '.claude', 'settings.json'),
      hookScript: 'C:\\Program Files\\CodeStatus\\resources\\hook\\hook.js',
      runtime: 'C:\\Program Files\\CodeStatus\\CodeStatus.exe',
      launcher: 'C:\\Users\\you\\AppData\\Local\\CodeStatus\\bin\\hook-claude-code.cmd',
      events: 14,
      problems: [],
    }
    : {
      installed: true,
      settingsPath: path.join(os.homedir(), '.claude', 'settings.json'),
      hookScript: '/opt/CodeStatus/resources/hook/hook.js',
