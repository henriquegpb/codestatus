...
//
// One file for both platforms, because the transport does not differ: Node's
// `net.connect` takes a Windows pipe name and a Unix socket path through the
// same call, and the daemon writes whichever it bound into a pointer file this
// reads. What does differ is where that pointer lives, which is the one branch
// below.
//
...
// off the critical path; it does not make it free. See desktop/README.md.
...
// Mirrors src/platform/<os>/paths.js. Kept here rather than required, see the
// header.
//
// The Linux branch reads $XDG_DATA_HOME and not $XDG_RUNTIME_DIR, and that is
// the important half of it. The daemon puts the socket in the runtime directory
// but the *pointer* to it here, precisely because this process may not have the
// same environment the app did: an agent started from a systemd unit, a
// container, or an ssh session without lingering has no $XDG_RUNTIME_DIR, and a
// hook that computed a different directory than the daemon would find no
// pointer, spool to a place nothing drains, and lose the events silently.
// $XDG_DATA_HOME falls back to a path derived from $HOME, which both programs
// always agree on.
function appDataDirectory() {
  ...
  return path.join(root, 'CodeStatus');
}

const runDir = path.join(appDataDirectory(), 'run');
...
// Mirrors hostFromEnvironment in src/platform/<os>/host.js.
//
// Only what can be proven; everything this cannot prove is left as unknown and
// resolved by the daemon, which walks the process tree.
//
// Windows has no TERM_PROGRAM convention and its terminals export almost
// nothing, and the tempting fallback there — reading PSModulePath and calling
// it PowerShell — is wrong: that variable is machine-wide on Windows 10 and
// later, so it is set in cmd, in VS Code, and in services alike.
//
// Linux is the opposite: its terminals export plenty, they just do not agree on
// what, so the list is long and each entry is one terminal's own idea of how to
// announce itself. VS Code has to come first on both — its integrated terminal
// inherits the variables of whatever terminal launched the editor, so a
// Konsole-launched VS Code sets KONSOLE_VERSION too.
const HOST_BY_VARIABLE = process.platform === 'win32'
  ...
  : [
    ...
    ['FOOT_PID', 'terminal'],
    ['GHOSTTY_RESOURCES_DIR', 'terminal'],
  ];
...
function detectHost() {
  ...
  // Some terminal claimed the variable, so we know it is one without knowing
  // which. Better than unknown, and the daemon's tree walk may still improve
  // on it. Windows never reaches this: nothing there sets TERM_PROGRAM.
  if (env.TERM_PROGRAM) return 'terminal';
  return 'unknown';
}
