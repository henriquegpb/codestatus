...
/**
 * Grainient — animated grainy gradient on a fullscreen WebGL triangle.
 *
 * Vendored from React Bits (`@react-bits/Grainient-TS-TW`) rather than pulled
 * through the shadcn CLI, which this project is not set up for. Kept as close
 * to upstream as possible so it can be diffed against a newer version -- the
 * only edits are two ternary statements rewritten as `if`/`else` to satisfy
 * `no-unused-expressions`. The site drives it entirely through props from
 * `site-background.tsx`.
 */
